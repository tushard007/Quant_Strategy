...
import org.factor_investing.quant_strategy.strategies.risk_adjusted_momentum.RiskAdjustedMomentumBacktestHistoryService;
import org.factor_investing.quant_strategy.strategies.risk_adjusted_momentum.RiskAdjustedMomentumBacktestService;
import org.factor_investing.quant_strategy.strategies.risk_adjusted_momentum.RiskAdjustedMomentumConstants;
...
@RestController
@RequestMapping("/api/risk-adjusted-momentum-backtest")
public class RiskAdjustedMomentumBacktestController {
    private final RiskAdjustedMomentumBacktestService service;
    private final RiskAdjustedMomentumBacktestHistoryService historyService;
    public RiskAdjustedMomentumBacktestController(RiskAdjustedMomentumBacktestService service, RiskAdjustedMomentumBacktestHistoryService historyService){this.service=service;this.historyService=historyService;}

    @PostMapping("/run/stock")
    public ResponseEntity<RiskAdjustedMomentumBacktestResult> run(@RequestParam @DateTimeFormat(iso=DateTimeFormat.ISO.DATE) LocalDate startDate,
      @RequestParam @DateTimeFormat(iso=DateTimeFormat.ISO.DATE) LocalDate endDate,
      @RequestParam(defaultValue="1000000") double initialCapital,@RequestParam(defaultValue="20") int entryRank,
      @RequestParam(defaultValue="40") int retentionRank,@RequestParam(defaultValue="NIFTY 500") String benchmark,
      ...
      @RequestParam(defaultValue = RiskAdjustedMomentumConstants.DEFAULT_REBALANCE_MODE) String rebalanceMode,
      ...
      @RequestParam(defaultValue = RiskAdjustedMomentumConstants.DEFAULT_STOP_MODEL) String stopModel,
      @RequestParam(defaultValue="0") double trailingStopPercent,
      @RequestParam(defaultValue="14") int atrPeriod,@RequestParam(defaultValue="3") double atrMultiplier,
      @RequestParam(defaultValue="2") int cooldownWeeks,@RequestParam(defaultValue="200") int benchmarkSmaPeriod,
      @RequestParam(defaultValue="20") double breadthThresholdPercent,@RequestParam(defaultValue="50") double weakExposureCapPercent){
        RiskAdjustedMomentumBacktestResult result=service.run(startDate,endDate,initialCapital,entryRank,retentionRank,benchmark,
                transactionCostPercent,slippagePercent,riskFreeRatePercent,rebalanceMode,bufferAmount,maximumLeverageAmount,
                borrowingInterestRatePercent,stopModel,trailingStopPercent,atrPeriod,atrMultiplier,cooldownWeeks,
    ...
    public RiskAdjustedMomentumBacktestResult execution(@PathVariable UUID runId){return historyService.get(runId);}

    @GetMapping("/export/{runId}")
    public ResponseEntity<byte[]> exportExecution(@PathVariable UUID runId){
        RiskAdjustedMomentumBacktestResult result=historyService.get(runId);
    ...
    private ResponseEntity<byte[]> excelResponse(RiskAdjustedMomentumBacktestResult result){
        return ResponseEntity.ok().header(HttpHeaders.CONTENT_DISPOSITION,"attachment; filename=risk-adjusted-momentum-backtest-"+result.startDate()+"-to-"+result.endDate()+".xlsx")
    ...
    private byte[] workbook(RiskAdjustedMomentumBacktestResult result){try(Workbook workbook=new XSSFWorkbook();ByteArrayOutputStream out=new ByteArrayOutputStream()){
        ...
        Sheet stops=workbook.createSheet("Stop Events");head(stops,header,"Ticker","Entry Date","Entry Price","Peak Price","Stop Level","Signal Date","Execution Date","Execution Price","Quantity","Realized P/L","Reentry Eligible Date");row=1;for(var item:result.stopOutEvents()){Row r=stops.createRow(row++);r.createCell(0).setCellValue(item.ticker());r.createCell(1).setCellValue(item.entryDate().toString());r.createCell(2).setCellValue(item.entryPrice());r.createCell(3).setCellValue(item.peakPrice());r.createCell(4).setCellValue(item.stopLevel());r.createCell(5).setCellValue(item.signalDate().toString());r.createCell(6).setCellValue(item.executionDate().toString());r.createCell(7).setCellValue(item.executionPrice());r.createCell(8).setCellValue(item.quantity());r.createCell(9).setCellValue(item.realizedProfitLoss());r.createCell(10).setCellValue(item.reentryEligibleDate().toString());}
        Sheet regime=workbook.createSheet("Regime Exposure");head(regime,header,"Signal Date","Breadth %","Benchmark Above SMA","Exposure Cap %","New Buys Allowed");row=1;for(var item:result.regimeExposureHistory()){Row r=regime.createRow(row++);r.createCell(0).setCellValue(item.signalDate().toString());r.createCell(1).setCellValue(item.breadthPercent());r.createCell(2).setCellValue(item.benchmarkAboveSma());r.createCell(3).setCellValue(item.exposureCapPercent());r.createCell(4).setCellValue(item.newBuysAllowed());}
        workbook.write(out);return out.toByteArray();}catch(Exception e){throw new IllegalStateException("Risk-adjusted momentum backtest Excel export failed",e);}}
