...
public class MomentumBacktestService {
    ...
    private void validate(LocalDate start,LocalDate end,double capital,int entry,int retention,double costs,double slippage){if(start==null||end==null||!start.isBefore(end))throw new IllegalArgumentException("Start date must be before end date");if(capital<=0)throw new IllegalArgumentException("Initial capital must be positive");if(entry<1||entry>50)throw new IllegalArgumentException("Entry rank must be between 1 and 50");if(retention<entry)throw new IllegalArgumentException("Retention rank must be at least the entry rank");if(costs<0||slippage<0)throw new IllegalArgumentException("Costs cannot be negative");}
